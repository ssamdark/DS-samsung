...
### 10-2-1. Size — 체크박스/라디오버튼 3단계
체크박스·라디오버튼 크기는 아래 Small/Medium/Large 3단계만 사용한다. **체크박스와 라디오버튼은 항상 동일 크기**를 쓴다. 현재는 전체 화면에서 Medium(16px)만 사용.

| 변수 | 값 | 용도 |
|------|-----|------|
| `--control-size-sm` | 14px | (예비 — 아직 미사용) |
| `--control-size-md` | 16px | ★ 기본 — 체크박스/라디오버튼 전체 적용 |
| `--control-size-lg` | 18px | (예비 — 아직 미사용) |
